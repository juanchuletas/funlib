...
int main() {
  ...
  flib::sycl_handler::select_device(
      "Intel"); // Selects a vendor for your computations
  flib::sycl_handler::get_device_info(); // Prints current device info

  int N = 5;
  std::vector<float> A = {4, 1, 0, 0, 0, 1, 4, 1, 0, 0, 0, 1, 4,
                          1, 0, 0, 0, 1, 4, 1, 0, 0, 0, 1, 3};
  ...
  float result = flib::tensor_operations::reduction(
      redvec, Q); // Reduction of a first order tensor (vector)
