                                ...
                                const std::vector<float>& weights)
{
    ...
    for(std::size_t batch = 0; batch < batch_size; batch++){
        for(std::size_t token = 0; token < token_count; token++){
            for(std::size_t output_feature = 0; output_feature < model_size; output_feature++){
                for(std::size_t input_feature = 0; input_feature < model_size; input_feature++){
                    output[(batch * token_count + token) * model_size + output_feature] +=
                        input[(batch * token_count + token) * model_size + input_feature] *
...
std::size_t projectedIndex(std::size_t batch,
                            std::size_t token,
                            std::size_t head,
                            std::size_t feature)
{
    return (batch * token_count + token) * model_size + head * head_size + feature;
}
                    ...
                    sycl::queue Q)
{
    const std::vector<std::size_t> expected_shape{
        batch_size, token_count, model_size};
...
int main()
{
    ...
    for(std::size_t i = 0; i < weightsQ.size(); i++){
        weightsQ[i] = static_cast<float>(static_cast<int>((i * 3 + 1) % 7) - 3) / 5.0f;
        weightsK[i] = static_cast<float>(static_cast<int>((i * 5 + 2) % 9) - 4) / 6.0f;
        weightsV[i] = static_cast<float>(static_cast<int>((i * 7 + 3) % 11) - 5) / 7.0f;
        weightsO[i] = static_cast<float>(static_cast<int>((i * 2 + 4) % 8) - 4) / 5.0f;
    }

    std::vector<float> expected =
        attentionReference(input, weightsQ, weightsK, weightsV, weightsO);
    flib::Tensor<float> X = deviceTensor(
        {batch_size, token_count, model_size}, input, Q);
    ...
    flib::Tensor<float> scores = flib::tensor_operations::gemm_batched(Qheads, Kheads, Q, false, true);
    flib::Tensor<float> scaled_scores = flib::operations::scale(scores, 1.0f / std::sqrt(static_cast<float>(head_size)), Q);
    flib::Tensor<float> probabilities = flib::operations::softmax(scaled_scores, Q);
    flib::Tensor<float> head_output = flib::tensor_operations::gemm_batched(probabilities, Vheads, Q);
    flib::Tensor<float> joined_output = flib::tensor_operations::permute(head_output, {0, 2, 1, 3}, Q);
    joined_output.reshape({batch_size, token_count, model_size});
    flib::Tensor<float> output = flib::tensor_operations::gemm(joined_output, Wo, Q);
