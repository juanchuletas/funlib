    ...
    for (std::size_t row = 0; row < rowsC; row++) {
      for (std::size_t col = 0; col < colsC; col++) {
        ...
        for (std::size_t k = 0; k < innerA; k++) {
          std::size_t indexA =
              transpose_A ? k * stored_colsA + row : row * stored_colsA + k;
          std::size_t indexB =
              transpose_B ? col * stored_colsB + k : k * stored_colsB + col;
          sum += A[batch * matrix_sizeA + indexA] *
                 B[batch * matrix_sizeB + indexB];
        }
...
bool compareResult(const std::string &name, const flib::Tensor<float> &result,
                   const ReferenceResult &expected, sycl::queue Q) {
...
int main() {
  ...
  passed =
      checkCase("normal matrices", {2, 2, 3}, {2, 3, 4}, false, false, Q) &&
      passed;
  passed =
      checkCase("transposed A", {2, 4, 2}, {2, 4, 3}, true, false, Q) && passed;
  passed =
      checkCase("transposed B", {2, 3, 2, 4}, {2, 3, 5, 4}, false, true, Q) &&
      passed;
  passed = checkCase("both transposed", {2, 4, 2}, {2, 3, 4}, true, true, Q) &&
           passed;
