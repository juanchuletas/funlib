...
bool checkResult(const std::string &name, const flib::Tensor<float> &result,
  ...
  for (std::size_t row = 0; row < actual.size() / row_size; row++) {
    ...
    if (std::abs(sum - 1.0f) > 2.0e-4f) {
      std::cerr << name << " row " << row << " does not sum to one"
                << std::endl;
      return false;
    }
  }
...
bool checkCase(const std::string &name, const std::vector<std::size_t> &shape,
               const std::vector<float> &values, sycl::queue Q) {
...
int main() {
  ...
  passed =
      checkCase("large stable values", {2, 3},
                {1000.0f, 1001.0f, 1002.0f, -1000.0f, -999.0f, -998.0f}, Q) &&
      passed;
  passed = checkCase("multiple tensor rows", {2, 2, 5},
                     {1.0f,  -2.0f, 3.0f,  0.0f,  2.0f, -1.0f, -2.0f,
                      -3.0f, -4.0f, -5.0f, 4.0f,  4.0f, 4.0f,  4.0f,
                      4.0f,  8.0f,  2.0f,  -6.0f, 1.0f, 0.0f},
                     Q) &&
           passed;
  ...
  passed =
      checkCase("rows longer than one work group", {2, 513}, long_row, Q) &&
      passed;
