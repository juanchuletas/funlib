...
template <typename T>
flib::Tensor<T> gemmReference(const flib::Tensor<T> &A,
                              const flib::Tensor<T> &B) {
...
template <typename T> void fillTensor(flib::Tensor<T> &tensor, int seed) {
...
template <> bool almostEqual<int>(int expected, int actual, std::size_t) {
  return expected == actual;
}

template <typename T, typename Operation>
bool checkMethod(const std::string &method, const flib::Tensor<T> &expected,
                 std::size_t rowsA, std::size_t colsA, std::size_t colsB,
                 sycl::queue Q, Operation operation) {
  try {
    flib::Tensor<T> actual = operation();

    if (actual.getRows() != rowsA || actual.getCols() != colsB) {
      std::cerr << method << " returned shape " << actual.getRows() << "x"
                << actual.getCols() << " instead of " << rowsA << "x" << colsB
                << std::endl;
      return false;
    }
    ...
    std::cerr << method << " threw an exception for " << rowsA << "x" << colsA
              << " * " << colsA << "x" << colsB << ": " << error.what()
              << std::endl;
    return false;
  }

  std::cout << "Passed " << method << " for " << rowsA << "x" << colsA << " * "
...
bool checkShape(std::size_t rowsA, std::size_t colsA, std::size_t colsB,
  ...
  passed = checkMethod<T>("gemm_naive", expected, rowsA, colsA, colsB, Q,
                          [&]() {
                            return flib::tensor_operations::gemm_naive(
                                deviceA, deviceB, Q);
                          }) &&
           passed;

  passed = checkMethod<T>("gemm dispatcher", expected, rowsA, colsA, colsB, Q,
                          [&]() {
                            return flib::tensor_operations::gemm(deviceA,
                                                                 deviceB, Q);
                          }) &&
           passed;

  passed = checkMethod<T>("gemmTiled", expected, rowsA, colsA, colsB, Q,
                          [&]() {
                            return flib::tensor_operations::gemmTiled(
                                deviceA, deviceB, Q);
                          }) &&
           passed;

  passed = checkMethod<T>("gemm_blocked2x2", expected, rowsA, colsA, colsB, Q,
                          [&]() {
                            return flib::tensor_operations::gemm_blocked2x2(
                                deviceA, deviceB, Q);
                          }) &&
           passed;

  passed =
      checkMethod<T>("gemm_tiled_blocked2x2", expected, rowsA, colsA, colsB, Q,
                     [&]() {
                       return flib::tensor_operations::gemm_tiled_blocked2x2(
                           deviceA, deviceB, Q);
                     }) &&
      passed;
...
bool checkInvalidMethod(const std::string &method, Operation operation) {
  try {
    ...
    std::cerr << method << " threw the wrong exception: " << error.what()
              << std::endl;
    return false;
  }
...
bool checkInvalidShapes(sycl::queue Q) {
  ...
  passed = checkInvalidMethod("gemm_naive",
                              [&]() {
                                flib::tensor_operations::gemm_naive(deviceA,
                                                                    deviceB, Q);
                              }) &&
           passed;
  ...
  passed = checkInvalidMethod("gemm_blocked2x2",
                              [&]() {
                                flib::tensor_operations::gemm_blocked2x2(
                                    deviceA, deviceB, Q);
                              }) &&
           passed;

  passed = checkInvalidMethod("gemm_tiled_blocked2x2",
                              [&]() {
                                flib::tensor_operations::gemm_tiled_blocked2x2(
                                    deviceA, deviceB, Q);
                              }) &&
           passed;
...
bool checkNDimensionalShape(sycl::queue Q) {
  ...
  flib::Tensor<float> result =
      flib::tensor_operations::gemm(deviceA, deviceB, Q);
  const std::vector<std::size_t> expected_shape{2, 3, 7};
  if (result.getShape() != expected_shape) {
    std::cerr << "GEMM did not preserve the leading tensor dimensions"
              << std::endl;
    return false;
  }

  std::cout << "Passed GEMM shape preservation for [2, 3, 5] * [5, 7]"
            << std::endl;
  return true;
}
...
bool checkReshape(sycl::queue Q) {
  ...
  if (hostTensor.getShape() != expected_shape || hostTensor[17] != 17.0f) {
    std::cerr
        << "Host tensor reshape changed its data or produced the wrong shape"
        << std::endl;
    return false;
  }
  ...
  if (deviceTensor.device_data() != original_pointer ||
      deviceTensor.getShape() != std::vector<std::size_t>{6, 4}) {
    std::cerr
        << "Device tensor reshape allocated memory or produced the wrong shape"
        << std::endl;
    return false;
  }
  ...
  std::cerr << "Tensor reshape accepted a different number of elements"
            << std::endl;
  return false;
}
...
bool checkBatchedGemm(sycl::queue Q) {
      ...
      for (std::size_t col = 0; col < 5; col++) {
        for (std::size_t k = 0; k < 4; k++) {
          expected[batch * 10 + row * 5 + col] +=
              hostA[batch * 8 + row * 4 + k] * hostB[batch * 20 + col * 4 + k];
        }
      }
    }
  }
  ...
  std::cout << "Passed batched GEMM for [2, 3, 2, 4] * [2, 3, 5, 4] transpose"
            << std::endl;
  return true;
}
