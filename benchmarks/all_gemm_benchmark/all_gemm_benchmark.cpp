...
double medianTotalTime(Operation operation, std::size_t warmups,
...
Measurements measure(std::size_t M, std::size_t K, std::size_t N,
  ...
  double total_time = medianTotalTime(total_operation, warmups, repetitions);
  double kernel_time = medianKernelTime(kernel_operation, warmups, repetitions);

  return Measurements{total_time, kernel_time,
                      calculateGflops(M, K, N, kernel_time)};
}

void printMeasurements(std::size_t M, std::size_t K, std::size_t N,
                       const std::string &method,
                       const Measurements &measurements,
                       double original_kernel_time) {
  std::cout << std::setw(5) << M << "x" << std::setw(5) << K << "x"
            << std::setw(5) << N << std::setw(28) << method << std::setw(18)
            << measurements.total_time << std::setw(18)
            << measurements.kernel_time << std::setw(18) << measurements.gflops
...
void benchmarkShape(std::size_t M, std::size_t K, std::size_t N,
  ...
  Measurements original = measure(
      ...
      [&](sycl::event &event) {
        auto C =
            flib::tensor_operations::gemm_naive(deviceA, deviceB, Q, &event);
        (void)C;
      });
  ...
  Measurements tiled = measure(
      ...
      [&](sycl::event &event) {
        auto C =
            flib::tensor_operations::gemmTiled(deviceA, deviceB, Q, &event);
        (void)C;
      });

  Measurements blocked = measure(
      M, K, N,
      [&]() {
  ...
  Measurements tiled_blocked = measure(
      M, K, N,
      [&]() {
        auto C =
  ...
  printMeasurements(M, K, N, "gemm_tiled_blocked2x2", tiled_blocked,
...
int main() {
  ...
  std::cout << "    M x    K x    N" << std::setw(28) << "Method"
            << std::setw(18) << "Total ms" << std::setw(18) << "Kernel ms"
            << std::setw(18) << "Kernel GFLOPS" << std::setw(18)
            << "GEMM speedup" << std::endl;
