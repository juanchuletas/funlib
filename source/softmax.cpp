...
Tensor<T> softmax(const Tensor<T> &input, sycl::queue Q,
  ...
  if (shape.size() == 0) {
    throw std::invalid_argument(
        "Softmax requires a tensor with at least one dimension");
  }
  ...
  if (input.is_device()) {
    if (!input.is_accessible_from(Q)) {
      throw std::invalid_argument(
          "Softmax queue cannot access the input tensor");
    }
      ...
      cgh.parallel_for(
          ...
          [=](sycl::nd_item<1> item) {
            ...
            for (std::size_t column = local_id; column < row_size;
              ...
              local_maximum =
                  sycl::max(local_maximum, input_data[row_offset + column]);
            }
            ...
            for (std::size_t column = local_id; column < row_size;
                 column += work_group_size) {
              T exponential =
                  sycl::exp(input_data[row_offset + column] - maximum);
      ...
      cgh.parallel_for(
          ...
          [=](sycl::nd_item<1> item) {
            ...
            for (std::size_t column = local_id; column < row_size;
                 column += work_group_size) {
              local_maximum =
                  sycl::max(local_maximum, input_accessor[row_offset + column]);
            }
            local_values[local_id] = local_maximum;
            item.barrier(sycl::access::fence_space::local_space);
            ...
            for (std::size_t column = local_id; column < row_size;
                 column += work_group_size) {
              T exponential =
                  sycl::exp(input_accessor[row_offset + column] - maximum);
...
Tensor<T> scaled_softmax(const Tensor<T> &input, T scale, sycl::queue Q,
  ...
  if (shape.size() == 0) {
    throw std::invalid_argument(
        "Scaled Softmax requires a tensor with at least one dimension");
  }

  std::size_t row_size = shape.back();
  if (row_size == 0) {
    throw std::invalid_argument(
        "Scaled Softmax final dimension cannot be zero");
  }
  ...
  if (input.is_device()) {
    if (!input.is_accessible_from(Q)) {
      throw std::invalid_argument(
          "Scaled Softmax queue cannot access the input tensor");
    }
          ...
          [=](sycl::nd_item<1> item) {
            ...
            for (std::size_t column = local_id; column < row_size;
              ...
              local_maximum = sycl::max(
                  local_maximum, input_data[row_offset + column] * scale);
            }
            local_values[local_id] = local_maximum;
            item.barrier(sycl::access::fence_space::local_space);
            ...
            for (std::size_t column = local_id; column < row_size;
                 column += work_group_size) {
              T exponential =
                  sycl::exp(input_data[row_offset + column] * scale - maximum);
              output_data[row_offset + column] = exponential;
              local_sum += exponential;
            }
            local_values[local_id] = local_sum;
      ...
      cgh.parallel_for(
          ...
          [=](sycl::nd_item<1> item) {
            ...
            for (std::size_t column = local_id; column < row_size;
                 column += work_group_size) {
              local_maximum = sycl::max(
                  local_maximum, input_accessor[row_offset + column] * scale);
            }
            local_values[local_id] = local_maximum;
            item.barrier(sycl::access::fence_space::local_space);
            ...
            for (std::size_t column = local_id; column < row_size;
                 column += work_group_size) {
              T exponential = sycl::exp(
                  input_accessor[row_offset + column] * scale - maximum);
              output_accessor[row_offset + column] = exponential;
              local_sum += exponential;
            }
            local_values[local_id] = local_sum;
...
template Tensor<float> softmax(const Tensor<float> &, sycl::queue,
                               sycl::event *);
template Tensor<double> softmax(const Tensor<double> &, sycl::queue,
                                sycl::event *);
template Tensor<float> scaled_softmax(const Tensor<float> &, float, sycl::queue,
                                      sycl::event *);
template Tensor<double> scaled_softmax(const Tensor<double> &, double,
