...
void flib::sycl_handler::select_device(std::string device_name,
  ...
  for (const auto &platform : sycl::platform::get_platforms()) {
    for (const auto &device : platform.get_devices()) {
      ...
      std::transform(devname.begin(), devname.end(), devname.begin(),
                     ::toupper);
      std::transform(device_name.begin(), device_name.end(),
                     device_name.begin(), ::toupper);

      bool name_matches = devname.find(device_name) != std::string::npos;
      bool type_matches =
          !filter_by_type ||
          (device.get_info<sycl::info::device::device_type>() == target_type);

      if (name_matches && type_matches) {
        _device = device;
        if (profiling) {
          _queue =
              sycl::queue(_device, sycl::property::queue::enable_profiling{});
...
void flib::sycl_handler::select_backend_device(
    const std::string &platform_filter, const std::string &device_filter) {
  std::string target_platform = platform_filter;
  std::string device_type_filter = device_filter;
  std::transform(target_platform.begin(), target_platform.end(),
                 target_platform.begin(), ::toupper);
  std::transform(device_type_filter.begin(), device_type_filter.end(),
                 device_type_filter.begin(), ::toupper);

  sycl::info::device_type target_type =
      device_type_from_string(device_type_filter);
  ...
  throw std::runtime_error(
      "SYCL: No matching platform/device found for platform '" +
...
void flib::sycl_handler::create_gl_interop_context(const std::string &name) {
  ...
  if (std::string(extensions).find("cl_khr_gl_sharing") == std::string::npos) {
    throw std::runtime_error(
        "OpenCL device does not support OpenGL interoperability.");
  }
  ...
  if (!_clCtx || err != CL_SUCCESS)
    throw std::runtime_error(
        "Failed to create OpenCL context for OpenGL interoperability.");
...
bool flib::sycl_handler::is_rtc_available(const std::string &name) {
  ...
  return q.get_device().ext_oneapi_can_compile(
  ...
  try {
    for (const auto &platform : sycl::platform::get_platforms()) {
      for (const auto &device : platform.get_devices()) {
        ...
        std::cout << "Max Work Item Sizes   : (" << max_work_item_sizes[2]
                  << ", " << max_work_item_sizes[1] << ", "
                  << max_work_item_sizes[0] << ")" << std::endl;

        // Removed invalid query: max_work_items_per_compute_unit
        // std::cout << "Max Work Items per CU : " <<
        // device.get_info<sycl::info::device::max_work_items_per_compute_unit>()
        // << std::endl;
...
flib::sycl_handler::device_type_from_string(const std::string &_type_str) {
...
void flib::sycl_handler::register_queue(const std::string &name,
                                        flib::device device_type,
                                        flib::vendor vendor_type,
                                        flib::backend backend_type,
                                        bool profiling) {
  // Validate vendor/backend combinations
  if (vendor_type == flib::vendor::NVIDIA &&
      (backend_type == flib::backend::OPENCL ||
       backend_type == flib::backend::LEVEL_ZERO))
    throw std::runtime_error(
        "NVIDIA does not support OpenCL or Level-Zero backends");
  ...
  std::string vendor_str =
      (vendor_type == flib::vendor::INTEL) ? "INTEL" : "NVIDIA";
  ...
  for (const auto &platform : sycl::platform::get_platforms()) {
    std::string pname = platform.get_info<sycl::info::platform::name>();
    std::transform(pname.begin(), pname.end(), pname.begin(), ::toupper);
    if (pname.find(backend_str) == std::string::npos)
      continue;
    ...
    for (const auto &dev : platform.get_devices()) {
      ...
      if (vname.find(vendor_str) == std::string::npos)
        continue;
      if (dev.get_info<sycl::info::device::device_type>() != target_type)
        continue;
      if (profiling) {
        _queues[name] =
            sycl::queue(dev, sycl::property::queue::enable_profiling{});
      } else {
  ...
  throw std::runtime_error("register_queue: no matching device found for '" +
                           name + "'");
}
...
sycl::queue flib::sycl_handler::get_queue() { return _queue; }
cl_context flib::sycl_handler::get_clContext() { return _clCtx; }
sycl::context flib::sycl_handler::get_sycl_context() { return _syclCtx; }
