...
template <typename T> Tensor<T>::Tensor(std::size_t rows) : Tensor({rows, 1}) {}
...
template <typename T> Tensor<T>::Tensor(const std::vector<std::size_t> &shape) {
  if (shape.size() == 0) {
    throw std::invalid_argument("Tensor shape cannot be empty");
  }
...
template <typename T> Tensor<T>::Tensor(const Tensor<T> &other) {
  ...
  if (other.is_device()) {
    ...
    queue.memcpy(m_device_data, other.m_device_data, m_gsize * sizeof(T))
        .wait();
  } else {
    m_data = std::make_unique<T[]>(m_gsize);
    for (std::size_t i = 0; i < m_gsize; ++i) {
...
template <typename T> Tensor<T>::Tensor(Tensor<T> &&other) noexcept {
  m_rows = other.m_rows;
  m_cols = other.m_cols;
  m_gsize = other.m_gsize;
...
template <typename T> Tensor<T>::~Tensor() { release_device_data(); }
template <typename T> void Tensor<T>::release_device_data() {
  if (m_device_data != nullptr && m_context.has_value()) {
    sycl::free(m_device_data, *m_context);
    m_device_data = nullptr;
  }
}
template <typename T> T &Tensor<T>::operator()(int row, int col) {
  if (is_device()) {
    throw std::runtime_error(
        "Device tensor data is not directly accessible from the host");
  }
...
template <typename T> const T &Tensor<T>::operator()(int row, int col) const {
  if (is_device()) {
    throw std::runtime_error(
        "Device tensor data is not directly accessible from the host");
  }
...
template <typename T> const T &Tensor<T>::operator[](int index) const {
  if (is_device()) {
    throw std::runtime_error(
        "Device tensor data is not directly accessible from the host");
  }
...
template <typename T> T &Tensor<T>::operator[](int index) {
  if (is_device()) {
    throw std::runtime_error(
        "Device tensor data is not directly accessible from the host");
  }
...
template <typename T> sycl::buffer<T, 1> Tensor<T>::to_sycl_buffer() const {
  if (is_device()) {
    throw std::runtime_error(
        "A device tensor cannot be wrapped in a host-backed SYCL buffer");
  }
...
sycl::event Tensor<T>::copy_from(const T *host_data, sycl::queue queue) {
  ...
  if (queue.get_context() != *m_context) {
    throw std::invalid_argument(
        "Queue context does not match the tensor context");
  }
...
Tensor<T> &Tensor<T>::reshape(const std::vector<std::size_t> &shape) {
  ...
  if (new_size != m_gsize) {
    throw std::invalid_argument(
        "The new tensor shape must contain the same number of elements");
  }
