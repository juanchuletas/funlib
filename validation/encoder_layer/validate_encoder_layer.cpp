...
Linear makeLinear(const std::vector<float> &weight,
  ...
  for (std::size_t out = 0; out < outputs; ++out) {
    for (std::size_t in = 0; in < inputs; ++in) {
      transposed[in * outputs + out] =
          weight.at(weight_offset + out * inputs + in);
    }
  }
...
Tensor linear(const Tensor &input, const Linear &parameters,
              sycl::queue queue) {
  auto projected =
      flib::tensor_operations::gemm(input, parameters.weight, queue);
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    const auto input_values =
        load(directory + "/ref_encoder_layer_input.bin", B * N * D);
    const auto expected =
        load(directory + "/ref_encoder_layer_output.bin", B * N * D);
    ...
    auto k_projection =
        makeLinear(packed_weight, packed_bias, D, D, queue, D * D, D);
    auto v_projection =
        makeLinear(packed_weight, packed_bias, D, D, queue, 2 * D * D, 2 * D);
    auto out_projection =
        makeLinear(weight("self_attn.out_proj.weight", {D, D}),
                   weight("self_attn.out_proj.bias", {D}), D, D, queue);
    auto expand =
        makeLinear(weight("linear1.weight", {hidden, D}),
    ...
    auto normalized1 =
        flib::operations::layer_norm(input, gamma1, beta1, epsilon, queue);
    ...
    auto normalized2 =
        flib::operations::layer_norm(residual1, gamma2, beta2, epsilon, queue);
